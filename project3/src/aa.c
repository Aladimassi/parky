...
void bubble_sort_parkings(Parking *parkings, int n) {
    for (int i = 0; i < n - 1; i++) {
...
void refresh_parking_treeview1(GtkWidget *parking_treeview) {
    ...
    Parking parkings[100];  // Vous pouvez ajuster la taille selon vos besoins
    int count = 0;
...
int check_parking_id(const char *parking_id, int threshold) {
    ...
    // Search for the parking ID
    for (int i = 0; i < count; i++) {
        if (strcmp(p_list[i].id, parking_id) == 0) {
            found = 1;

            // Check if the number of spaces meets the threshold
            if (p_list[i].nbrspaces < threshold) {
                printf("Parking ID %s has fewer than %d available spaces.\n", parking_id, threshold);
                return 0;  // Not enough spaces
            }

            // Deduct the reserved spaces
            p_list[i].nbrspaces -= threshold;
            printf("Parking ID %s updated with %d spaces remaining.\n", parking_id, p_list[i].nbrspaces);

            // Write the updated list back to the file
            file = fopen("parking.txt", "w");
            if (!file) {
                perror("Failed to open parking.txt for writing");
                return 0;
            }
            for (int j = 0; j < count; j++) {
                fprintf(file, "%s %d\n", p_list[j].id, p_list[j].nbrspaces);
            }
            fclose(file);
